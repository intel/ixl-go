...
import (
	"github.com/intel/ixl-go/compress/internal/codelencode"
...
type dynamicHeader struct {
	...
	histogram   []int32 // length symbol frequency or lengths
	rcodes      []uint16
	countCache  []int32
}
...
func newDynamicHeader() *dynamicHeader {
	return &dynamicHeader{
		...
		histogram:  make([]int32, 19),
		rcodes:     make([]uint16, 19),
		source:     make([]uint8, 286+30+1),
		countCache: make([]int32, (7+1)*2),
	}
}

func (c *dynamicHeader) writeTo(histogram *iaa.Histogram, eos bool, b *headerFrame) {
	...
	for i := 0; i < len(c.data); i++ {
		value := c.data[i]
		b.WriteBit(c.rcodes[value], int8(c.histogram[value]))
		switch value {
		case numRepeat3_6:
			i++
			extra := c.data[i]
...
func (c *dynamicHeader) prepareAlphabets(histogram *iaa.Histogram) {
	litNum, distanceNum := codelencode.Prepare(histogram, c.source)
	if distanceNum == 0 {
		c.source[litNum] = 1
		distanceNum = 1
	}
...
func (c *dynamicHeader) numRepeat(num byte, repeated int) {
	for repeated != 0 {
		...
		case repeated <= 3:
			...
			c.histogram[num] += int32(repeated)
			repeated = 0
		case repeated <= 7:
			c.histogram[num]++
			c.data = append(c.data, num)
